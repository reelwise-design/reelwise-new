    ...
    */
    ...
    function careerFocusedBiography(sourceBio, person) {
      ...
      const careerTerms =
        /\b(actor|actress|filmmaker|film|films|movie|movies|role|roles|starred|starring|debut|breakthrough|recognition|acclaim|performance|performances|career|directed|director|produced|producer|screen|academy award|golden globe|bafta|emmy|cannes|venice|sundance|theatre|theater)\b/i;
      ...
      const educationOnly =
        /\b(graduated|college|university|school|degree|studied|education|moved to|worked in|worked for)\b/i;

      const chosen = [];

      // Keep the opening identity sentence when it identifies the performer.
      if (sentences[0] && careerTerms.test(sentences[0])) {
        chosen.push(sentences[0]);
      }

      // Then preserve source order and choose career-defining material.
      for (const sentence of sentences) {
        if (chosen.includes(sentence)) continue;
        if (personalTerms.test(sentence)) continue;

        const career = careerTerms.test(sentence);
        const education = educationOnly.test(sentence);

        // Education/background is useful only when the same sentence also
        // reaches the person's acting/film career.
        if (!career || (education && !/\b(acting|actor|actress|film|debut|role|career)\b/i.test(sentence))) {
          continue;
        }

        chosen.push(sentence);

        // Four source sentences is the Reelwise target. This normally gives
        // breakthrough + defining work + later career without encyclopedia length.
        if (chosen.length >= 4) break;
      }

      // If the source was unusually sparse, fill from its first complete sentences
      // rather than inventing biography facts.
      if (chosen.length < 2) {
        for (const sentence of sentences) {
          if (chosen.includes(sentence) || personalTerms.test(sentence)) continue;
          chosen.push(sentence);
          if (chosen.length >= 3) break;
        }
      }

      let bio = chosen.join(" ").replace(/\s+/g, " ").trim();

      // Keep the phone profile compact while NEVER cutting a sentence in half.
    ...
    async function getPersonProfile(personId) {
      /*
        Keep TMDB as the profile/credits source. Wikipedia supplies richer
        biography text when available, but Reelwise now condenses that source
        into a short, chronological, movie-career-focused profile.
      */
